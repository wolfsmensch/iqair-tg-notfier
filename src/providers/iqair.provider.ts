...
export class IqAirProvider {
     ...
     */
    private static readonly indexValueCssSelector = '#main-content [class*="aqi-box-shadow-"] p.text-lg.font-medium';

    /**
     * Значение User-Agent для запроса HTML страницы
     */
    private static readonly requestUserAgent = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36';
     ...
     */
    public async getIndexByCityUrl(cityUrl: string): Promise<number> {
     ...
     */
    private async _getCityPageHtml(cityUrl: string): Promise<string> {
     ...
     */
    private _getIndexFromCityPageHtml(pageHtml: string): number {
